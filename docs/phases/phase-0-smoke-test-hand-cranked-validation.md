...
10. Once the beans are cool and rested, look at them and record a [roast level](../glossary.md#roast-level) as `roast_level_observed`. Ten seconds of work, and it is the only reason Phase 3 will later be able to tell a working vision grader from a lucky one. See [roast-log-schema.md](../roast-log-schema.md).
